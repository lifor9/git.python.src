...
for mm in range(1,9):
    ...
    fname = "avguserproc_{0}{1}_142.sql.sql".format('19', str(mm).zfill(2))
    f = open(fname, 'w')
    month_end = calendar.monthrange(2019,mm)[1]
    # print(month_end)

    insert_sql = "INSERT INTO avguserproc_{0}{1}{2}00 \n"
    # select_sql = "SELECT _ontunetime+31536000,_agenttime+31536000,_agentid,_userid,_usr,_sys,_usrsys,_sz,_rss,_vmem,_chario,_processcnt,_threadcnt "
    # select_sql = select_sql + "FROM avguserproc_{0}{1}{2}00 "
    # select_sql = select_sql + "WHERE _agentid in ({3}) ORDER BY _ontunetime; \n"
    select_sql = "SELECT _ontunetime+{0},_agenttime+{0},"
    select_sql = select_sql + "_agentid,_userid,_usr,_sys,_usrsys,_sz,_rss,_vmem,_chario,_processcnt,_threadcnt "
    select_sql = select_sql + "FROM avguserproc_1803{1}00 "
