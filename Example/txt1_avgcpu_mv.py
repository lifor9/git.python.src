...
for mm in range(1,9):
    ...
    fname = "avgcpu_{0}{1}_286.sql".format('19', str(mm).zfill(2))
    f = open(fname, 'w')
    month_end = calendar.monthrange(2019,mm)[1]
    # print(month_end)

    insert_sql = "INSERT INTO avgcpu_{0}{1}{2}00 \n"
    # select_sql = "SELECT _ontunetime+31536000,_agenttime+31536000,"
    # select_sql = select_sql + "_agentid,_index,_user,_sys,_wait,_idle,_runqueue,_fork,_exec,_interupt,_systemcall,_contextswitch "
    # select_sql = select_sql + "FROM avgcpu_{0}{1}{2}00 "
    # select_sql = select_sql + "WHERE _agentid in ({3}) ORDER BY _ontunetime; \n"
    select_sql = "SELECT _ontunetime+{0},_agenttime+{0},"
    select_sql = select_sql + "286,_index,_user,_sys,_wait,_idle,_runqueue,_fork,_exec,_interupt,_systemcall,_contextswitch "
    select_sql = select_sql + "FROM avgcpu_1803{1}00 "
